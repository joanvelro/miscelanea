...
def dl_ia_cla_plot_descriptive_model(df_aux2, figures_path, road ):
    """
    ...
    try:
        ...
        fig.add_trace(go.Scatter(x=df_aux2['TIME'],
                                 ...
                                 name='descriptive'))
        for day in df_aux2['WEEKDAY'].unique():
            fig.add_trace(go.Scatter(x=df_aux2[df_aux2['WEEKDAY'] == day]['TIME'],
                                     y=df_aux2[df_aux2['WEEKDAY'] == day]['INTENSITY'],
                                     mode='markers+lines',
        ...
        fig.update_yaxes(title_text='Intensity (veh/h)')
        fig.update_xaxes(title_text='Time')
        fig.update_layout(title='Descriptive model - {} - Section:{} - Weekday:{}'.format(road, section, weekday))
        fig.show()
        fig.write_html(figures_path + "\\descriptive_model_check_without_month_{}.html".format(road))

    except Exception as exception_msg:
        logger.error('(!) {} Error in plot descriptive model (without month): {}'.format('-' * 20, exception_msg))
